...
window.dhtmlXScheduler = window.scheduler = {
    ...
    function() {
        ...
        dhx_cal_data: function(e) {
            ...
            setTimeout(function(){
                str=String(window.location.href);                
                split=str.split("/");
                ArrayLength=split.length;   
                $(".dhx_cal_ltext option[value="+split[ArrayLength-1]+"]").attr("selected","selected").prependTo(".dhx_cal_ltext select");
            },10);
        },
        ...
        labels: {
            ...
            icon_details: "შეცვლა",
            icon_edit: "Edit",
            icon_delete: "გაუქმება",
            confirm_closing: "",
            confirm_deleting: "გსურთ ჯავშნის გაუქმება ?",
    ...
    function() {
        ...
        // m += '<div class="dhx_event_move dhx_title" style="' + u + g + '">' + r + "</div>", m += '<div class="dhx_body" style=" width:' + (s - (this._quirks ? 4 : 14)) + "px; height:" + (a - (this._quirks ? 20 : 30) + 1) + "px;" + u + g + '">' + d + r + "</div>";
        m += '<div class="dhx_event_move dhx_title" style=" width:' + (s - (this._quirks ? 4 : 8)) + "px; height:" + (a - (this._quirks ? 20 : 10)) + "px;" + u + g + '">' + d + r + "</div>";
    ...
    scheduler._init_lightbox_events = function() {
        ...
        if (!this._lightbox) {
            ...
            for (var t = this._lightbox_template, i = this.config.buttons_left, s = 0; s < i.length; s++){
                if(!s)t+="<div class='addEventButton'>";
                t += "<div class='dhx_btn_set dhx_left_btn_set " + i[s] + "_set'><div dhx_button='1' class='" + i[s] + "'></div><div>" + scheduler.locale.labels[i[s]] + "</div></div>";
            }
            i = this.config.buttons_right;
            for (var s = 0; s < i.length; s++) t += "<div class='dhx_btn_set dhx_right_btn_set " + i[s] + "_set'><div dhx_button='1' class='" + i[s] + "'></div><div>" + scheduler.locale.labels[i[s]] + "</div></div>";
            t+="</div>";
